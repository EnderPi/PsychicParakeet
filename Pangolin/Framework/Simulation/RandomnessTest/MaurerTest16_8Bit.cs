...
{
    public class MaurerTest16_8Bit : IIncremental16RandomTest
    {
        ...
        public void Process(ushort randomNumber)
        {
            ...
            numbers[0] = randomNumber >> 8;
            numbers[1] = randomNumber & 255;
            for (int i = 0; i < 2; i++)
            {
                UInt64 difference = _iterationsPerformed - _indices[numbers[i]];
                _indices[numbers[i]] = _iterationsPerformed;
                if (_isInitialized)
                {
