...
{
    public class GorillaTest16 : IIncremental16RandomTest
    {
        private UInt32[][] _counts;
        private int _wordSize;
        private BitByBitUShorts _words;
        ...
        public GorillaTest16(int wordSize)
        {
            _wordSize = wordSize;
        ...
        public void CalculateResult(bool detailed)
        {
            ...
            _aggregatePValue = TestHelper.ChiSquaredForPValues(_pValues);
            GetTestResults();
        ...
        {
            _counts = new uint[16][];
            _pValues = new double[16];
            _testResults = new TestResult[16];
            for (int i = 0; i < 16; i++)
            {
                _counts[i] = new uint[_cellCount];
            }
            _overallTestResult = TestResult.Inconclusive;
            _words = new BitByBitUShorts(_wordSize);
        }
